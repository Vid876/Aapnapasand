      ...
      <div className="container-app">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-6">
          {PROCESS_STEPS.map((step, index) => (
            <article key={step.title} className={`group overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-stone-950/5 transition duration-300 hover:-translate-y-1 hover:shadow-xl ${index < 3 ? "lg:col-span-2" : "lg:col-span-3"}`}>
              <div className="relative aspect-[4/3] bg-stone-100">
                <Image src={step.image} alt={step.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" />
                <span className="absolute left-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-sm font-bold text-[#173f4f] shadow-md">{index + 1}</span>
              </div>
              <div className="p-5">
